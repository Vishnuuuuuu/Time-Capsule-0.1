...
async function sendEmail(to, capsuleName, filePath) {
  ...
  files.forEach(async file => {
    try {
      ...
      if (maturityDate === today.toISOString().split('T')[0]) {
        await sendEmail(userEmail, userCapsuleName, file.name);
      } else if (new Date(maturityDate) < yesterday) { // Check if maturity date is before yesterday
        await file.delete(); // Deleting capsules that have exceeded their maturity date
